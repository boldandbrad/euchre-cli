...
class Card():
    ...
    def is_left_bower(self, trump_suit: Suit) -> (bool, Suit):
        # if jack of same color as trump suit
        if self.face.name == 'Jack' and self.suit.color == trump_suit.color and \
                self.suit.name != trump_suit.name:
            return True, trump_suit
        return False, self.suit
    ...
    def weighted_value(self, trump_suit: Suit, lead_suit: Suit) -> int:
        ...
        """
        left_bower, adjusted_suit = self.is_left_bower(trump_suit)

        weighted_value = self.face.value  # 9, 10, 11, 12, 13, 14

        if left_bower:
            weighted_value += 15  # 26
        elif adjusted_suit.name == trump_suit.name:
        ...
        elif trump_suit.name != lead_suit.name and adjusted_suit.name == lead_suit.name:
