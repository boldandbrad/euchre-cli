...
def hand(hand_num: int, players: List[Player], teams: List[Team]) -> None:
    ...
    while not trump_suit:
        ...
        if not trump_suit:
            output(f"Everyone has passed. {players[-1].name} will now re-deal.")

        # log player hands and deck
        for player in players:
            logger.debug(f"{player.name}'s hand: {[str(c) for c in player.hand]}")
...
def trick(
    hand_num: int, trick_num: int, players: List[Player], trump_suit: Suit
) -> List[Card]:
